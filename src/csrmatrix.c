...
csr_matrix csr_matrix_convert_from_en2(int njuncs, 
   ...
   for (col = 1; col <= njuncs; col++) {
      matrix->columns[i_nnz] = col;
      matrix->values[i_nnz] = Aii[col];
      
      int start = XLNZ[col];
      int stop = XLNZ[col+1];
      
      matrix->rowIndex[col-1] = i_nnz+1;
      i_nnz++;
      for (i = start; i < stop; i++) {
         matrix->columns[i_nnz] = NZSUB[i];
         matrix->values[i_nnz] = Aij[LNZ[i]];
         i_nnz++;
      }
...
void csr_matrix_solve(csr_matrix matrix, double *B, double *X) {
   ...
   pardiso (matrix->handle, &matrix->maxfct, &matrix->mnum, &matrix->mtype, &phase,
            &matrix->n, matrix->values, matrix->rowIndex, matrix->columns, 0/*perm*/, &matrix->nrhs,
            matrix->iparm, &matrix->msglvl, B, X, &error,  matrix->dparm);
